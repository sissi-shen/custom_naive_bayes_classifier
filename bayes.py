...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    V['__unknown__'] = 0
    unique_words = set()
    for words_list in neg + pos:
        for word in words_list:
            unique_words.add(word)
    unique_list = sorted(list(unique_words))

    for index, w in enumerate(unique_list, start=1):
        V[w] = index
...
def vectorize_docs(docs:Sequence, V:dict) -> np.ndarray:
    ...
    document_vectors = []
    for doc in docs:
        row_vector = vectorize(V, doc)
        document_vectors.append(row_vector)
    D = np.array(document_vectors)
    return D
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        self.prior_0 = np.sum(y == 0) / ndocs
        self.prior_1 = np.sum(y == 1) / ndocs

        self.likelihood_0 = (np.sum(X[y == 0], axis=0) + 1) / (np.sum(X[y == 0]) + nwords)
        self.likelihood_1 = (np.sum(X[y == 1], axis=0) + 1) / (np.sum(X[y == 1]) + nwords)
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        for doc in range(len(X)):
            class_0 = np.log(self.prior_0) + (X[doc] @ np.log(self.likelihood_0))
            class_1 = np.log(self.prior_1) + (X[doc] @ np.log(self.likelihood_1))
            pred = [class_0, class_1]
            # print(pred)
